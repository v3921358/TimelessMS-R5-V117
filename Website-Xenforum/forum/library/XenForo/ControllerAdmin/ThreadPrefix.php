...
class XenForo_ControllerAdmin_ThreadPrefix extends XenForo_ControllerAdmin_Abstract
{
	...
	protected function _getPrefixGroupOrError($prefixGroupId)
	{
		...
		if (!$info)
		{
			throw $this->responseException($this->responseError(new XenForo_Phrase('requested_thread_prefix_group_not_found'), 404));
		}
	...
	protected function _getPrefixOrError($prefixId)
	{
		...
		if (!$info)
		{
			throw $this->responseException($this->responseError(new XenForo_Phrase('requested_thread_prefix_not_found'), 404));
		}
